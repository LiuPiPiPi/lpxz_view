	...
	<div class="form">
		...
		<el-form :inline="true" :model="commentForm" :rules="formRules" ref="formRef" size="small">
			...
			<div class="el-form-item el-form-item--small emoji">
				<img src="https://fastly.jsdelivr.net/gh/Naccl/article-resource/img/paopao/1.png" @click="showEmojiBox">
				...
				<div class="emoji-box" v-show="emojiShow">
					...
					<div class="emoji-tabs">
						<a class="tab-link" :class="{'on':activeEmojiTab===0}" @click="activeEmojiTab=0">
							<img src="https://fastly.jsdelivr.net/gh/Naccl/article-resource/img/tv/1.png">
						</a>
						<a class="tab-link" :class="{'on':activeEmojiTab===1}" @click="activeEmojiTab=1">
							<img src="https://fastly.jsdelivr.net/gh/Naccl/article-resource/img/aru/1.png">
						</a>
						<a class="tab-link" :class="{'on':activeEmojiTab===2}" @click="activeEmojiTab=2">
							<img src="https://fastly.jsdelivr.net/gh/Naccl/article-resource/img/paopao/1.png">
						</a>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			postForm() {
				...
				const articleToken = window.localStorage.getItem(`article${this.commentQuery.articleId}`)
				this.$refs.formRef.validate(valid => {
					if (!valid || this.commentForm.content === '' || this.commentForm.content.length > 250) {
						...
						this.$store.dispatch('submitCommentForm', articleToken ? articleToken : '')
					}
				})
			}
		}
